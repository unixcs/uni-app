...
对有运行行为的页面、API、权限、数据库、配置、支付或订单改动，检查通过后还不能直接提交 GitHub。AI 必须遵循 [WSL 本地验证与腾讯云生产上线工作流](../WSL本地验证与腾讯云生产上线工作流.md)：

```text
自动检查通过
→ 部署到 WSL 本地运行环境
→ 用户明确回复本地手工验收通过
→ 单独申请 Git commit/push 授权
→ push 后执行 production preflight/dry-run
→ 单独申请腾讯云生产授权
→ 发布并验证生产
```

AI 每次报告必须准确使用以下状态，不能互相替代：

- **源码修改完成**：只说明工作区源码已变；
- **WSL 本地上线完成**：本地实际运行目录/进程/获批的本地数据已更新；
- **GitHub 推送完成**：commit 已到远端，不代表腾讯云已更新；
- **腾讯云生产发布完成**：新 release 已激活并通过生产证据验证。

用户批准开始修复后，可以执行安全、可逆的本地 build、静态产物替换、缓存修复和必要服务重启；本地数据库写入/清理、`.env` 或敏感配置修改必须先说明影响并另行确认。Git push 与腾讯云部署始终分别授权。

纯文档、纯测试、纯注释等无可部署运行行为的改动可记录理由后豁免 WSL 人工验收；边界不明时默认不豁免。对于 WSL 无法完整复现的真实回调、生产证书/白名单或生产密钥场景，完成所有可行验证并列出剩余风险，只有用户明确接受后才继续。紧急通道只能由用户针对重大生产影响显式启用，不能取消最小检查、回滚准备、独立 Git/生产授权和上线后验证。

### 7.7 收尾

确认验证、WSL 验收或豁免证据、提交边界、任务状态和归档。活跃任务应代表真实工作队列，已完成任务不要长期留在 `in_progress`。
