...
## Acceptance Criteria
- [ ] 单元/集成测试覆盖命令失败、危险路径、脏工作树、域名漂移、敏感文件和回滚。
- [ ] 干净提交可重复产生含清单/校验值的完整运行包。
- [ ] 模拟远端发布/失败回滚通过，不需要 GitHub/npm/Composer 服务器网络。
- [ ] test/prod 小程序产物只含对应域名。
- [ ] Skill 通过 quick_validate 并遵守双授权。
