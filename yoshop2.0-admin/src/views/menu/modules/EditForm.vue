...
export default {
  ...
  methods: {
    ...
    setFieldsValue () {
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.record, ['name', 'path', 'module_key', 'is_page', 'parent_id', 'sort']))
      })
    },
    ...
    formatTreeDataForMenuList (list, disabled = false) {
      ...
      list.forEach(item => {
        ...
        if (item.children && item.children.length) {
          newItem['children'] = this.formatTreeDataForMenuList(item['children'], newItem.disabled)
        }
        data.push(newItem)
      })
      return data
